...
class PaymentService:
    SECRET_KEY = "HOLI-HUB-SECURE-SALT" # In production, move to env var

    @staticmethod
    def generate_secure_reference():
        """
        Generates a secure payment reference: HOLI-{timestamp}-{random6}
        Plus a short hash for verification to prevent tampering.
        """
        timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
        random_str = ''.join(random.choices(string.ascii_uppercase + string.digits, k=6))
        base_ref = f"HOLI-{timestamp}-{random_str}"
    ...
    def verify_reference_integrity(reference):
        ...
        if len(parts) < 4: # HOLI-TIMESTAMP-RANDOM-HASH
            return False
    ...
    def create_pending_payment(supabase, payment_data):
        ...
        payment_data.setdefault('store_name', 'Holi Store')
